...
class DeformConv2D(nn.Module):
    ...
    def forward(self, x, offset):
        ...
        # (b, c, h, w, N)
        x_q_lt = self._geK_feat_q(x, q_lt, N)
        x_q_rb = self._geK_feat_q(x, q_rb, N)
        x_q_lb = self._geK_feat_q(x, q_lb, N)
        x_q_rt = self._geK_feat_q(x, q_rt, N)
    ...
    def _geK_feat_q(self, x, q, N):
        ...
        # (b, h, w, N)
        index = q[..., :N]*padded_w + q[..., N:]  # offseK_feat*w + offset_y
        # (b, c, h*w*N)
...
class temporal_fusion(nn.Module):
    def __init__(self, dim: int, n_head: int, num_frames: int, attn_mask: torch.Tensor = None):
        ...
        self.IK_q = nn.Linear(dim, all_head_dim, bias=False)
        self.IK_q_bias = nn.Parameter(torch.zeros(all_head_dim))
        self.IK_kv = nn.Linear(dim, all_head_dim * 2, bias=False) # 197 tokens(cls+patch) * num_frames
        self.IK_kv_bias = nn.Parameter(torch.zeros(all_head_dim * 2))
        
        self.KI_proj = nn.Linear(all_head_dim, dim)
        self.attn_mask = attn_mask
    
    def fusion(self, I_feat, K_feat): # I_feat=[n (b t) d], K_feat=[b n d]
        B, _, _ = K_feat.shape
        t = I_feat.shape[1] // K_feat.shape[0]
        I_feat_pat = I_feat[1:, :, :]
        # I_feat_pat = rearrange(I_feat_pat, 'n b d -> b n d') # batch -> token
        I_feat_pat = I_feat_pat + self.clip_space_pos
        # K_feat = rearrange(K_feat, 'b (t n) d -> (b t) n d', t=t)
        K_feat = K_feat + self.vmae_space_pos
        IK_q_bias = self.IK_q_bias
        IK_kv_bias = self.IK_kv_bias
        
        IK_q = F.linear(input=K_feat, weight=self.IK_q.weight, bias=IK_q_bias)
        # IK_q = rearrange(IK_q, 'b n (h d) -> b h n d', h=self.num_head)
        IK_kv = F.linear(input=I_feat_pat, weight=self.IK_kv.weight, bias=IK_kv_bias)
        # IK_kv = rearrange(IK_kv, 'b n (e h d) -> e b h n d',e=2, h=self.num_head)
        IK_k, IK_v = IK_kv[0], IK_kv[1]
        
        IK_q = IK_q * self.scale
        IK_attn = (IK_q @ IK_k.transpose(-2, -1))
        
        IK_attn = IK_attn.softmax(dim=-1)
        
        K_feat = (IK_attn @ IK_v)
        # K_feat = rearrange(K_feat, 'b h t d -> b t (h d)')
        K_feat = self.KI_proj(K_feat)
        # K_feat = rearrange(K_feat, '(b t) n d -> b (t n) d', b=B)
        return K_feat

    def forward(self, I_feat: torch.Tensor, K_feat: torch.Tensor):
        return self.fusion(I_feat, K_feat)
...
class spatial_fusion(nn.Module):
    def __init__(self, dim: int, n_head: int, num_frames: int, attn_mask: torch.Tensor = None):
        ...
        self.KI_q = nn.Linear(dim, all_head_dim, bias=False) # 197 tokens(cls+patch) * num_frames
        self.KI_q_bias = nn.Parameter(torch.zeros(all_head_dim))
        self.KI_kv = nn.Linear(dim, all_head_dim * 2, bias=False)
        self.KI_kv_bias = nn.Parameter(torch.zeros(all_head_dim * 2))
    ...
    def fusion(self, I_feat, K_feat): # I_feat=[n (b t) d], K_feat=[b n d]
        B, _, _ = K_feat.shape
        t = I_feat.shape[1] // K_feat.shape[0]
        I_feat_cls, I_feat_pat = I_feat[0, :, :], I_feat[1:, :, :]
        # I_feat_pat = rearrange(I_feat_pat, 'n (b t) d -> (b n) t d', b=B) # batch -> token
        I_feat_pat = I_feat_pat + self.clip_time_pos
        # K_feat = rearrange(K_feat, 'b (t n) d -> (b n) t d', t=t)
        K_feat = K_feat + self.vmae_time_pos
        KI_q_bias = self.KI_q_bias
        KI_kv_bias = self.KI_kv_bias
        
        KI_q = F.linear(input=I_feat_pat, weight=self.KI_q.weight, bias=KI_q_bias)
        # KI_q = rearrange(KI_q, 'b t (h d) -> b h t d', h=self.num_head)
        KI_kv = F.linear(input=K_feat, weight=self.KI_kv.weight, bias=KI_kv_bias)
        # KI_kv = rearrange(KI_kv, 'b t (e h d) -> e b h t d',e=2, h=self.num_head)
        KI_k, KI_v = KI_kv[0], KI_kv[1]
        
        KI_q = KI_q * self.scale
        KI_attn = (KI_q @ KI_k.transpose(-2, -1))
        
        KI_attn = KI_attn.softmax(dim=-1)
        
        I_feat_pat = (KI_attn @ KI_v)
        # I_feat_pat = rearrange(I_feat_pat, 'b h n d -> b n (h d)')
        I_feat_pat = self.KI_proj(I_feat_pat)
        # I_feat_pat = rearrange(I_feat_pat,'(b n) t d -> n (b t) d', b=B)
        I_feat = torch.cat([I_feat_cls.unsqueeze(0), I_feat_pat], dim=0)
        return I_feat
...
class EGMDL(nn.Module):
    ...
    def forward(self, x, pids=None, camid=None):    # concat[image, event, keypoint_feature]
        _, _t, _, _, _ = x.size()    
        
        
        key_point = x[:,2*_t//3:,:,:,:]
        b, t, c, w, h = key_point.size()    
        key_point = key_point.contiguous().view(b * t, c, w, h)  
        ...
        event_feature = F.relu(self.deformconv(event, offsets))     
        event_feature = event_feature.contiguous().view(b, t, c, w, h)
        
                
        _x = x[:,0:_t//3,:,:,:]
        b, t, c, w, h = _x.size()    
        _x = _x.contiguous().view(b * t, c, w, h)  

        #############################################################################
        #############################################################################
        #############################################################################
        feat_map = self.base(_x)  
        keypoint_feat = self.base(key_point)
        
        spatial_feat =  spatial_fusion(keypoint_feat,feat_map)
        temporal_feat = temporal_fusion(keypoint_feat,feat_map)

        sp_feat_map = torch.concat((spatial_feat,temporal_feat),dim=1)


        event_f, img_f = cross_alignment(event_feature,feat_map)
        feat_map = torch.concat((event_f,img_f),dim=1)

        feat_map = torch.concat((sp_feat_map,feat_map),dim=1)
        #############################################################################
        #############################################################################
        #############################################################################
